...
class Trainer:
    ...
    def prediction_loop(
        ...
        evaluate_mlm_acc = False
        if type(self.model) == BertForMaskedLM:
            evaluate_mlm_acc = True
            total_correct = 0
            total_masked = 0

        disable_tqdm = not self.is_local_process_zero() or self.args.disable_tqdm
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only and not evaluate_mlm_acc)
            batch_size = inputs[list(inputs.keys())[0]].shape[0]
            if loss is not None:
                eval_losses.extend([loss] * batch_size)
            if logits is not None and not prediction_loss_only: # Shape batch_size x seq_len x num_tokens
                preds = logits if preds is None else nested_concat(preds, logits, dim=0)
            if labels is not None and not prediction_loss_only: # Shape batch_size x seq_len, value -100 if the index is not masked.
            ...
            if evaluate_mlm_acc:
                ...
                for example in range(labels.shape[0]):
                    for token in range(labels.shape[1]):
                        if labels[example][token].item() == -100: # Not masked.
                            continue
                        total_masked += 1
                        if labels[example][token] == predictions[example][token]:
                            total_correct += 1
        if evaluate_mlm_acc:
            mlm_accuracy = float(total_correct) / float(total_masked)
    ...
    def prediction_step(
        ...
        logits = tuple(logit.detach() for logit in logits if type(logit) == torch.Tensor)
        if len(logits) == 1:
            logits = logits[0]
