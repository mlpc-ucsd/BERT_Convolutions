...
# Can generate examples with dependencies for MLM.
@dataclass
class DataCollatorForDependencyLM(DataCollatorForLanguageModeling):
    def __call__(
        self, examples: List[Union[List[int], torch.Tensor, Dict[str, torch.Tensor]]]
    ) -> Dict[str, torch.Tensor]:
        if len(examples[0]) == 2:
            # Dependency-supervised step.
            # Examples is a list of (token_id list, dependency tensor) tuples
            token_id_batch = [example[0] for example in examples]
            tokens_batch = self._tensorize_batch(token_id_batch)
            inputs, labels = self.mask_tokens(tokens_batch)
            dependency_labels_batch = [example[1] for example in examples]
            dependencies_batch = self._tensorize_batch(dependency_labels_batch)
            attention_mask = inputs != self.tokenizer.pad_token_id
            return {"input_ids": inputs, "attention_mask": attention_mask, "labels": labels, "dependency_labels": dependencies_batch}
        else:
            # Normal MLM step.
            # Examples is a list of token_id lists.
            token_id_batch = examples
            tokens_batch = self._tensorize_batch(token_id_batch)
            inputs, labels = self.mask_tokens(tokens_batch)
            attention_mask = inputs != self.tokenizer.pad_token_id
            return {"input_ids": inputs, "attention_mask": attention_mask, "labels": labels}
...
class DataCollatorForSOP(DataCollatorForLanguageModeling):
    ...
    def __call__(self, examples: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        if "sentence_order_label" in examples[0]:
